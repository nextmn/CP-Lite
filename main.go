...
func main() {
	...
	app := &cli.Command{
		Name:                  "cp-lite",
		Usage:                 "NextMN-CP Lite - Experimental Control Plane Simulator",
		...
		Flags: []cli.Flag{
			&cli.StringFlag{
				...
				Sources: cli.NewValueSourceChain(
					...
					clixdg.ConfigFile("nextmn-cp-lite/config.yaml"),
				),
			},
		},
		DefaultCommand: "run",
		Commands: []*cli.Command{
			...
			},
			{
				...
				Action: func(ctx context.Context, cmd *cli.Command) error {
					...
					if err := healthcheck.NewHealthcheck(*conf.Control.Uri.JoinPath("status"), "go-github-nextmn-cp-lite").Run(ctx); err != nil {
						os.Exit(1)
					}
					return nil
				},
